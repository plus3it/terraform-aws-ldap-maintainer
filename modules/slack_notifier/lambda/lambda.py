...
if not os.environ.get("AWS_EXECUTION_ENV"):
    log_file_name = "slack_notifier.log"
...
class SlackMessageBuilder:
    ...
    def __init__(
        ...
        self.days_since_pwdlastset = days_since_pwdlastset
    ...
    def _get_artifact_urls_block(self):
        text = (
            ...
            f"\n\t greater than {self.days_since_pwdlastset} days:"
            f" {self.user_counts[str(self.days_since_pwdlastset)]}"
        )
        human_readable = ""
        machine_readable = ""
    ...
    def _get_context_block(self):
        return {
            "type": "context",
            "elements": [
                {"type": "mrkdwn", "text": f"Report Generated: {self.report_time}"}
            ],
        }
    ...
    def _get_buttons_block(self):
        return {"type": "actions", "elements": self._get_buttons()}

    def _get_buttons(self):
        actions = ["deny", "approve"]
        buttons = []
        button_value = json.dumps(
            {"task_token": self.task_token, "ldap_scan_results": self.ldap_scan_results}
        )
        for action in actions:
            if action == "approve":
                ...
                style = "danger"
            buttons.append(self._get_button(action.capitalize(), button_value, style))
        return buttons
...
def build_slack_user_message(event):
    ...
    message_body = SlackMessageBuilder(
        ...
        days_since_pwdlastset=event["days_since_pwdlastset"],
    )
...
def build_slack_response_message(original_blocks, msg):
    ...
    updated_blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": msg}})
...
def handler(event, context):
    ...
    if event.get("message_to_slack"):
        ...
        slack_message = build_slack_response_message(
            original_blocks=original_blocks, msg=message
        )
        send_updated_message_to_slack(
            channel_id=channel_id, timestamp=timestamp, message_blocks=slack_message
        )
    else:
        event["days_since_pwdlastset"] = os.environ["DAYS_SINCE_PWDLASTSET"]
